...
class UploadFotoActivity : AppCompatActivity() {
    ...
    fun connectServer(v: View?) {
        ...
        Toast.makeText(this,"Mohon Tunggu 10 Detik Sampai Selesai", Toast.LENGTH_SHORT).show()
...
//                        finish()
    ...
    fun updateData(status: String){
        ...
        if(extras != null) {
            val nik = extras.getString(UploadFotoActivity.KEY)
            Log.d("MAIN_ES", "$nik")
            if (nik != null) {
                Log.d("MAIN_ES", "$nik")
    ...
    fun checkPermission(permission: String, requestCode: Int){
        if (ContextCompat.checkSelfPermission(this@UploadFotoActivity, permission) == PackageManager.PERMISSION_DENIED) {

            // Requesting the permission
            ActivityCompat.requestPermissions(this@UploadFotoActivity, arrayOf(permission), requestCode)
        } else {
            Toast.makeText(this@UploadFotoActivity, "Permission already granted", Toast.LENGTH_SHORT).show()
        }
    }
